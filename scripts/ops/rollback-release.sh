...
if [ "\${DEPLOYMENT_STRATEGY}" = "blue-green" ] && [ -z "\${TARGET_RELEASE}" ]; then
  current_dir="\$(readlink -f "\${REMOTE_DEPLOY_ROOT}/current")"
  REMOTE_DEPLOY_ROOT="\${REMOTE_DEPLOY_ROOT}" COMPOSE_PROJECT="\${COMPOSE_PROJECT}" HOST_BIND="\${HOST_BIND}" HOST_PORT="\${HOST_PORT}" BLUE_GREEN_ALTERNATE_PORT="\${BLUE_GREEN_ALTERNATE_PORT}" PROXY_ALIAS="\${PROXY_ALIAS}" BLUE_GREEN_PROXY_CONTAINER="\${BLUE_GREEN_PROXY_CONTAINER}" BLUE_GREEN_PROXY_CONFIG_PATH="\${BLUE_GREEN_PROXY_CONFIG_PATH}" BLUE_GREEN_HOST_PATTERN="\${BLUE_GREEN_HOST_PATTERN}" PUBLIC_HEALTHCHECK_URL="\${PUBLIC_HEALTHCHECK_URL}" bash "\${current_dir}/scripts/ops/blue-green-rollback-remote.sh"
  exit 0
fi
