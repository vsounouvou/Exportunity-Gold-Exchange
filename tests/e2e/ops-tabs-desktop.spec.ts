...
async function loginToApp(page: any) {
  ...
  if (!authResponse.ok()) {
    const fallbackEmail = `ops-tabs-desktop-${Date.now()}@exportunity.local`;
    const registerResponse = await page.request.post("/api/ece/auth/register", {
      data: {
        email: fallbackEmail,
        password: "ChangeMe123!",
        displayName: "Ops Tabs Desktop E2E",
      },
    });
...
test("desktop app-pro routes expose responsive controls", async ({ page }) => {
  ...
  await page.getByTestId("chat-primary-card").click();
  await expect(page).toHaveURL(/\/app\/chats\/general-operations(\?|$)/);

  await page.goto("/app/actions", { waitUntil: "domcontentloaded" });
  await expect(page.getByTestId("actions-feed")).toBeVisible();
  await page.getByTestId("actions-filter-orders").click();
  await expect(page).toHaveURL(/\/app\/actions\?filter=orders/);
  await page.getByTestId("actions-filter-all").click();
  ...
  await expect(page).toHaveURL(/\/app\/wallet\/send(\?|$)/);
  await expect(page.getByTestId("wallet-send-panel")).toBeVisible();
});
