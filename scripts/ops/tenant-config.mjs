...
function printShell(config) {
  const scalarEntries = {
    ...
    DEPLOYMENT_STRATEGY: config.deploymentStrategy ?? "replace",
    ...
    BLUE_GREEN_ALTERNATE_PORT: config.blueGreenAlternatePort ?? "",
    BLUE_GREEN_PROXY_CONTAINER: config.blueGreenProxyContainer ?? "",
    BLUE_GREEN_PROXY_CONFIG_PATH: config.blueGreenProxyConfigPath ?? "",
    BLUE_GREEN_HOST_PATTERN: config.blueGreenHostPattern ?? "",
