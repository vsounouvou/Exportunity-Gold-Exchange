...
CREATE INDEX IF NOT EXISTS finance_accounts_tenant_provider_idx
  ON finance_accounts (tenant_id, provider);
...
CREATE INDEX IF NOT EXISTS finance_events_tenant_provider_occurred_idx
  ON finance_events (tenant_id, provider, occurred_at DESC);

CREATE UNIQUE INDEX IF NOT EXISTS finance_events_tenant_provider_event_uidx
  ON finance_events (tenant_id, provider, provider_event_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS finance_transactions_provider_tx_type_uidx
  ON finance_transactions (tenant_id, provider, provider_tx_id, tx_type, direction);
...
CREATE INDEX IF NOT EXISTS finance_transactions_tenant_status_idx
  ON finance_transactions (tenant_id, status);

CREATE INDEX IF NOT EXISTS finance_transactions_tenant_currency_idx
  ON finance_transactions (tenant_id, currency);

CREATE INDEX IF NOT EXISTS finance_transactions_provider_tx_idx
  ON finance_transactions (provider, provider_tx_id);
...
CREATE INDEX IF NOT EXISTS finance_daily_metrics_tenant_date_idx
  ON finance_daily_metrics (tenant_id, metric_date DESC);
