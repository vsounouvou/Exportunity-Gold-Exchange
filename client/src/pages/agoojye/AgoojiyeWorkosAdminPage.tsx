...
function assistantActionDescription(action: any) {
  if (action.output?.summary) return action.output.summary;
  if (action.status === "awaiting_approval") return "Exécution bloquée jusqu’à approbation.";
  if (action.status === "approved" && action.requiresApproval) return "Action approuvée. Toute exécution reste séparée et auditée.";
  if (action.status === "denied") return "Action refusée. Aucune exécution n’a eu lieu.";
  if (action.status === "failed") return "L’action n’a pas pu être menée à terme.";
  return "Action interne enregistrée.";
}
...
function AssistantPanel({ data, refetch }: { data: AdminOverview; refetch: () => void }) {
  ...
  const approve = useMutation({
    mutationFn: (actionId: number) => apiRequest(`/api/admin/agoojye/workos/assistant/actions/${actionId}/approve`, { method: "POST", body: "{}" }),
    onSuccess: () => {
      assistantQuery.refetch();
      refetch();
      toast({ title: "Action approuvée", description: "La validation humaine a été enregistrée dans le journal de sécurité." });
    },
    onError: (error: any) => toast({ title: "Approbation impossible", description: error?.message, variant: "destructive" }),
  });
