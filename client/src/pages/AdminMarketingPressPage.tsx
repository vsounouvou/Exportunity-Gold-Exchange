...
export default function AdminMarketingPressPage() {
  ...
  const queryResult = useQuery({
    queryKey: ["admin-marketing-press", query],
    queryFn: async () => {
      const suffix = query ? `?status=all&limit=200&q=${encodeURIComponent(query)}` : "?status=all&limit=200";
      const response = await apiRequest(`/api/admin/marketing/press${suffix}`, { method: "GET" });
      return Array.isArray(response?.items) ? (response.items as PressRow[]) : [];
    },
  });

  const items = queryResult.data || [];

  const syncForm = (item: PressRow | null) => {
    if (!item) {
      setForm({ title: "", slug: "", outlet: "", excerpt: "", externalUrl: "", thumbnailLocal: "", tags: "", status: "draft", sortOrder: "0", publishedAt: "" });
      setImagePrompt("");
      return;
    }
    setForm({
    ...
    setImagePrompt(`Press thumbnail for ${item.title}. Outlet: ${item.outlet || "media"}.`);
  };

  const assetKey = useMemo(() => {
    const slug = slugify(form.slug || form.title, selectedId ? `press-${selectedId}` : "new-press");
    return `marketing/press/${slug}/thumbnail`;
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (selectedId) return apiRequest(`/api/admin/marketing/press/${selectedId}`, "PATCH", payload);
      return apiRequest("/api/admin/marketing/press", "POST", payload);
    },
    onSuccess: async () => {
      toast({ title: "Saved", description: "Press item saved." });
      await queryClient.invalidateQueries({ queryKey: ["admin-marketing-press"] });
      setSelectedId(null);
      syncForm(null);
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: number) => apiRequest(`/api/admin/marketing/press/${id}`, { method: "DELETE" }),
    onSuccess: async () => {
      toast({ title: "Deleted", description: "Press item deleted." });
      await queryClient.invalidateQueries({ queryKey: ["admin-marketing-press"] });
      setSelectedId(null);
      syncForm(null);
    },
  ...
  const suggestPrompt = async () => {
    ...
    try {
      const res = await apiRequest("/api/admin/assets/images/suggest-prompt", "POST", {
        ...
        assetKey,
        description: `Press thumbnail for ${form.title}. ${form.excerpt}`,
        aspect: "16:9",
      });
  ...
  const generateThumbnail = async () => {
    ...
    try {
      ...
      if (nextPath) {
        ...
        toast({ title: "Thumbnail generated", description: "Image generated and linked to this press card." });
      }
    } catch (error: any) {
      ...
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-6 lg:grid-cols-[360px,1fr]">
        <Card className="border-gray-800 bg-gray-900">
          <CardHeader>
            ...
            <Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search press" className="border-gray-700 bg-gray-950" />
            <Button onClick={() => { setSelectedId(null); syncForm(null); }}>New Press Item</Button>
          </CardHeader>
          <CardContent className="max-h-[70vh] space-y-2 overflow-auto">
            {items.map((item) => (
              <button key={item.id} onClick={() => { setSelectedId(item.id); syncForm(item); }} className={`w-full rounded-lg border p-3 text-left transition-colors ${selectedId === item.id ? "border-sky-400 bg-sky-500/10" : "border-gray-800 bg-gray-950 hover:bg-gray-900"}`}>
                ...
                <div className="mt-1 text-xs text-gray-400">{item.outlet || "Outlet"}</div>
              </button>
            ))}
          </CardContent>
        </Card>
        ...
        <Card className="border-gray-800 bg-gray-900">
          <CardHeader>
            <CardTitle>{selectedId ? `Edit Press #${selectedId}` : "Create Press Item"}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <Textarea value={form.excerpt} onChange={(event) => setForm((prev) => ({ ...prev, excerpt: event.target.value }))} placeholder="Excerpt" className="min-h-[140px] border-gray-700 bg-gray-950" />
