...
import { 
  Store, MessageSquare, Send, ChevronRight, CheckCircle2, 
...
const APPLICATION_STEPS = [
  { id: 1, title: "Welcome", description: "Introduction to becoming a shop owner" },
  { id: 2, title: "Business Info", description: "Tell us about your business" },
  { id: 3, title: "Products", description: "What will you sell?" },
  { id: 4, title: "Experience", description: "Your trading experience" },
  { id: 5, title: "Verification", description: "Identity and documents" },
  { id: 6, title: "Review", description: "Submit for AI assessment" },
];
  ...
  useEffect(() => {
    if (!existingApplication) {
      const welcomeMessage: Message = {
        ...
        role: "assistant",
        content: `Welcome to ${brand.name} — Seller application.

This application assistant guides the submission process for seller access.
...
To begin: what is your company name?`,
  ...
  const sendMessageMutation = useMutation({
    mutationFn: async (userMessage: string) => {
      const response = await apiRequest('/api/admin/applications/shop/chat', {
        method: 'POST',
        body: JSON.stringify({
          message: userMessage,
          currentStep,
  ...
  const submitApplicationMutation = useMutation({
    mutationFn: async () => {
      const response = await apiRequest('/api/admin/applications/shop', {
        method: 'POST',
        body: JSON.stringify({
          applicationData,
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <Card className="bg-gray-900 border-gray-800">
            <CardHeader className="text-center">
              ...
              <CardDescription>
                {existingApplication.status === 'approved' 
                  ? "Congratulations! You're now a verified shop owner."
                  : existingApplication.status === 'rejected'
                  ? "Unfortunately, your application was not approved at this time."
                  : "Your application is being reviewed. We'll notify you of the decision."}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              {existingApplication.status === 'approved' && (
                <Link href="/seller-dashboard">
                  <Button className="w-full h-11 mt-4 bg-amber-500 hover:bg-amber-600">
      ...
      <header className="border-b border-gray-800 bg-gray-900/50 backdrop-blur-sm sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-4 py-3">
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <h1 className="text-lg font-semibold text-white">Shop Owner Application</h1>
          </div>
          ...
          {currentStep >= 5 && Object.keys(applicationData).length >= 4 ? (
            ...
            <div className="flex gap-2">
              ...
              <Button
                onClick={handleSend}
                disabled={!input.trim() || isLoading}
                className="h-11 w-11 bg-amber-500 hover:bg-amber-600 p-0"
              >
                {isLoading ? (
                  <Loader2 className="h-4 w-4 animate-spin" />
                ) : (
