...
/** Schema-only runtime parity. It never starts an interview, render, or worker. */
export async function ensureMediaStudioTables() {
  ...
  await db.execute(sql`
    create table if not exists media_interview_claims (
      ...
      constraint media_interview_claims_confidence_check check (confidence_bps between 0 and 10000),
  ...
  await db.execute(sql`
    ...
    $$ language plpgsql
  `);
  await db.execute(sql`drop trigger if exists media_interview_events_immutable on media_interview_events`);
  await db.execute(sql`
  ...
  `);
  await db.execute(sql`drop trigger if exists media_studio_events_immutable on media_studio_events`);
  await db.execute(sql`
