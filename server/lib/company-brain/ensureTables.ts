...
export async function ensureCompanyBrainTables() {
  ...
  `);
  await db.execute(sql`create index if not exists company_brain_source_connectors_tenant_service_idx on company_brain_source_connectors (tenant_id, service, status);`);
  await db.execute(sql`create unique index if not exists company_brain_source_connectors_connection_service_uniq on company_brain_source_connectors (connection_id, service);`);
  ...
  `);
  await db.execute(sql`create index if not exists company_brain_oauth_states_tenant_expiry_idx on company_brain_oauth_states (tenant_id, expires_at);`);
  ...
  `);
  await db.execute(sql`create index if not exists company_brain_sync_runs_connector_created_idx on company_brain_sync_runs (connector_id, created_at);`);
  await db.execute(sql`create index if not exists company_brain_sync_runs_tenant_status_idx on company_brain_sync_runs (tenant_id, status, created_at);`);
  ...
  `);
  await db.execute(sql`create unique index if not exists company_brain_sync_cursors_connector_type_uniq on company_brain_sync_cursors (connector_id, cursor_type);`);
  ...
  `);
  await db.execute(sql`create index if not exists company_brain_sync_dead_letters_connector_status_idx on company_brain_sync_dead_letters (connector_id, status, created_at);`);
  ...
  await db.execute(sql`
    create table if not exists company_brain_sources (
      ...
      tenant_id integer not null references tenants(id) on delete cascade,
      company_id integer references companies(id) on delete cascade,
      connector_id integer references company_brain_source_connectors(id) on delete set null,
  ...
  `);
  await db.execute(sql`alter table company_brain_sources add column if not exists connector_id integer;`);
  await db.execute(sql`
    do $$
    begin
  ...
  await db.execute(sql`create index if not exists company_brain_sources_connector_status_idx on company_brain_sources (connector_id, status);`);
