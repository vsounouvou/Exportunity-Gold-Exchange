...
export default function MarketingMediaPressPage() {
  if (!isExportunityMarketingHost()) return <Redirect to="/zone" />;

  const [query, setQuery] = useState("");
  const mediaQuery = useQuery({
    queryKey: ["marketing-press-page", query],
    queryFn: () => fetchMarketingMedia({ limit: 180, q: query || undefined, type: "article,press_release,profile,post" }),
  });
  ...
  return (
    <MarketingShell active="proof">
      <MarketingContainer className="pt-10 md:pt-14">
        <HeroPanel image={marketingSiteConfig.images?.platform} imageAlt="Press coverage">
          <div className="max-w-3xl space-y-4">
            <MarketingKicker>MEDIA / PRESS</MarketingKicker>
            <MarketingTitle className="text-4xl md:text-5xl">Press Coverage</MarketingTitle>
          </div>
        </HeroPanel>
      </MarketingContainer>
      ...
      <MarketingContainer className="py-8">
        <div className="mb-5 w-full md:w-72">
          <Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search press" className="border-white/20 bg-white/10 text-white" />
        </div>
          ...
          {visible.map((item: any) => (
            <a key={String(item.id)} href={item.url} target="_blank" rel="noreferrer" className="group rounded-2xl border border-white/10 bg-black/30 p-3 transition-all hover:-translate-y-0.5 hover:border-white/30">
              ...
              <div className="p-2 pt-4">
                <div className="text-xs uppercase tracking-[0.12em] text-white/60">{item.outlet || "Press"}</div>
                ...
                {item.excerpt ? <div className="mt-2 text-sm text-white/70 line-clamp-3">{item.excerpt}</div> : null}
              </div>
            </a>
          ))}
        </div>
